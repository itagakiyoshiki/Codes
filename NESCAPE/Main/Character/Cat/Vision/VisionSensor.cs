...
public class VisionSensor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //視界の全てのネズミがLossならばLoss地点との距離を測る
        float _discoverLostMouseDis = 0.0f;
        if (discoverObjectList.CheckAllDiscoverMousesLossFlags())
        {
            _discoverLostMouseDis = Vector3.Distance(
                transform.position, discoverCompass.DiscoverLostMousePos);
        }

        //全てのネズミがLossの状態で最後に見た地点に行った場合Lossしたとする
        if (discoverObjectList.CheckAllDiscoverMousesLossFlags() && _discoverLostMouseDis <= huntEndRange)
        {
            mouseLoss = true;
            //リスト内にLoss中のやつがいても見失ったところに来たらリストを消す
            discoverObjectList.RemoveInVisionMouses();
