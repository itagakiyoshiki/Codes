...
public class DiscoverCompass : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            //視界内のネズミがすべてLossになった場合最後に見た地点へ向かう
            if (discoverObjectList.CheckAllDiscoverMousesLossFlags())
            {
                discoverLostMousePos = discoverObjectList.DiscoverLostTransform.position;
                return;
            }
    ...
    public bool GetCheckAllDiscoverMousesLossFlags()
    {
        return discoverObjectList.CheckAllDiscoverMousesLossFlags();
    }
