...
public class AssetVision : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        if (show_two_levels)
        {
            ...
            far_cone.transform.SetParent(gameObject.transform, false);
            far_cone.transform.position = transform.position;
        ...
        vision_angle = catSetting.VisionAngle;
    }
    ...
    private void Update()
    {
        ...
        if (gameStart)
        {
            ...
            vision_range = nowVisionSetting.VisionDistance;
        }
        ...
        if (timer > refresh_rate)
        {
            ...
            UpdateMainLevel(mesh, nowVisionSetting.NearVisionDistance);

            if (show_two_levels)
                UpdateFarLevel(mesh_far, 0.1f, nowVisionSetting.VisionDistance);
        }
    }
    ...
    private void UpdateMainLevel(MeshFilter mesh, float range)
    {
        ...
        vision_angle = nowVisionSetting.VisionAngle;
        int minmax = Mathf.RoundToInt(vision_angle / 2f);
        float step_jump = Mathf.Clamp(vision_angle / precision, 0.01f, minmax);
        for (float i = -minmax; i <= minmax; i += step_jump)
        {
